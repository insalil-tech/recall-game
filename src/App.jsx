...
// ─────────────────────────────────────────────────────────────────────────────
// ADMIN PASSWORD — change "recall2025" to anything you like
// ─────────────────────────────────────────────────────────────────────────────
const ADMIN_PASSWORD = "recall2025";

// ─────────────────────────────────────────────────────────────────────────────
// PERPETUAL DATE-SEEDED CHALLENGE GENERATOR
// Takes today's date → produces a unique, consistent challenge forever.
// Same date = same puzzle for every player in the world. No database needed.
// ─────────────────────────────────────────────────────────────────────────────
function dateToSeed(d) {
  // Combines year + month + day into a unique integer per calendar day
...
// Card decks — 8 themed sets of 18 icons each
const DECKS = {
  animals: ["🐶","🐱","🐸","🐼","🦊","🐧","🐨","🦁","🐯","🦋","🐢","🦄","🐬","🦒","🐘","🦓","🦜","🐙"],
  food:    ["🍕","🍔","🍟","🌮","🍜","🍣","🍩","🍦","🎂","🍇","🍓","🍑","🥑","🫐","🍋","🥝","🍒","🍆"],
  nature:  ["🌸","🌺","🌻","🌹","🌷","🍀","🌴","🌵","🌲","🍁","🌊","🌈","⛅","❄️","🌙","⭐","🌍","🔥"],
...
// Difficulty rules: day-of-week biases difficulty, seed picks the rest
...
const LABELS = ["Morning Puzzle","Brain Boost","Daily Flip","Memory Test","Mind Stretch",
                "Focus Time","Think Fast","Sharp Mind","Card Quest","Flip Master",
                "Recall Challenge","Pair Hunt","Quick Match","Memory Lane","Pattern Play"];
...
// ─────────────────────────────────────────────────────────────────────────────
// LOCAL ANALYTICS — stores in browser, visible in admin dashboard
// (Replace with Supabase later for global data across all players)
// ─────────────────────────────────────────────────────────────────────────────
function dayKey(date) { return `recall_${date.toISOString().slice(0,10)}`; }

function saveEvent(date, event, data = {}) {
  try {
    const k = dayKey(date);
    const rec = JSON.parse(localStorage.getItem(k) || "{}");
    if (!rec.events) rec.events = [];
    rec.events.push({ event, data, ts: Date.now() });
    if (event === "start")    rec.starts = (rec.starts || 0) + 1;
    if (event === "abandon")  rec.abandons = (rec.abandons || 0) + 1;
    if (event === "complete") {
      rec.completions = (rec.completions || 0) + 1;
      rec.times = [...(rec.times || []), data.time];
      rec.moves_list = [...(rec.moves_list || []), data.moves];
    }
...
function loadAllAnalytics() {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (k?.startsWith("recall_")) {
      try { out.push({ date: k.replace("recall_",""), ...JSON.parse(localStorage.getItem(k)) }); } catch {}
    }
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// SIMULATED GLOBAL SCORES (deterministic per challenge seed)
// Makes result screen feel real even with 0 actual other players.
// When you add Supabase, replace this with real DB queries.
// ─────────────────────────────────────────────────────────────────────────────
function simulateStats(ch, yourTime, yourMoves) {
  const rand = rng(ch.seed + 55555);
  ...
  return (
    <div style={{position:"fixed",inset:0,pointerEvents:"none",overflow:"hidden",zIndex:300}}>
      {Array.from({length:50},(_,i)=>{
        const l=Math.random()*100, dl=Math.random()*1.5, dr=2+Math.random()*2;
        const c=colors[i%colors.length], s=7+Math.random()*9;
        return <div key={i} style={{position:"absolute",left:`${l}%`,top:"-20px",width:s,height:s,
          background:c,borderRadius:Math.random()>.5?"50%":"2px",
          animation:`cf ${dr}s ${dl}s linear infinite`}}/>;
      })}
      <style>{`@keyframes cf{0%{transform:translateY(-20px) rotate(0deg);opacity:1}
        100%{transform:translateY(100vh) rotate(720deg);opacity:0}}`}</style>
    </div>
  );
}
...
// ─────────────────────────────────────────────────────────────────────────────
// ADMIN DASHBOARD
// Access: tap the 🃏 logo 5 times on the landing screen
// ─────────────────────────────────────────────────────────────────────────────
function AdminDashboard({ onClose }) {
  const [pw, setPw] = useState(""), [auth, setAuth] = useState(false), [data, setData] = useState([]);
  useEffect(()=>{ if(auth) setData(loadAllAnalytics()); },[auth]);

  const attempt = () => pw===ADMIN_PASSWORD ? setAuth(true) : alert("Wrong password — try again");
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:400}}>
      <div style={{background:"var(--color-background-primary)",borderRadius:16,padding:28,width:290,textAlign:"center"}}>
        ...
        <input type="password" placeholder="Password" value={pw}
          ...
          style={{width:"100%",padding:"10px 12px",borderRadius:8,border:"1.5px solid var(--color-border-tertiary)",
  ...
  const tot = { starts: data.reduce((a,d)=>a+(d.starts||0),0), completions: data.reduce((a,d)=>a+(d.completions||0),0), abandons: data.reduce((a,d)=>a+(d.abandons||0),0) };
  const allTimes = data.flatMap(d=>d.times||[]);
  const compRate = tot.starts ? Math.round(tot.completions/tot.starts*100) : 0;
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:400}}>
      <div style={{background:"var(--color-background-primary)",borderRadius:"18px 18px 0 0",padding:"20px 16px 36px",
        width:"100%",maxWidth:480,maxHeight:"88vh",overflowY:"auto",boxSizing:"border-box"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {[["🎮","Total Starts",tot.starts],["✅","Completions",tot.completions],
            ["📉","Abandoned",tot.abandons],["📈","Completion %",compRate+"%"],
            ["⏱","Best Time",allTimes.length?fmt(Math.min(...allTimes)):"—"],
            ["⏱","Avg Time",allTimes.length?fmt(avg(allTimes)):"—"]
          ].map(([ic,lbl,val])=>(
            <div key={lbl} style={{background:"var(--color-background-secondary)",borderRadius:10,padding:"11px 10px",textAlign:"center"}}>
              ...
              <div style={{fontSize:19,fontWeight:700,color:"var(--color-text-primary)"}}>{val}</div>
            </div>
          ))}
        </div>
        ...
        {data.length===0
          ? <p style={{fontSize:13,color:"var(--color-text-secondary)",textAlign:"center",padding:"24px 0"}}>No data yet — play a round to see stats here!</p>
          : data.map(d=>(
            <div key={d.date} style={{background:"var(--color-background-secondary)",borderRadius:10,padding:"11px 12px",marginBottom:7}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                ...
                <span style={{fontSize:11,color:"var(--color-text-secondary)"}}>{d.completions||0}/{d.starts||0} done</span>
              </div>
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                {d.times?.length ? <>
                  <span style={{fontSize:11,color:"var(--color-text-secondary)"}}>⏱ Best {fmt(Math.min(...d.times))}</span>
                  <span style={{fontSize:11,color:"var(--color-text-secondary)"}}>Avg {fmt(avg(d.times))}</span>
        ...
        <button onClick={onClose} style={{width:"100%",marginTop:12,padding:12,borderRadius:10,border:"none",
          background:"var(--color-background-secondary)",color:"var(--color-text-secondary)",fontSize:14,cursor:"pointer"}}>Close</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  // Reset game state when entering game screen
  useEffect(()=>{
    if(screen==="game"){
      setCards(buildCards(ch)); setFlipped([]); setMatched(0);
      setMoves(0); setElapsed(0); setStarted(false); setLocked(false);
    }
  },[screen]);
  ...
  // Track abandon on tab close
  useEffect(()=>{
    const h=()=>{ if(started&&matched<ch.pairs) saveEvent(now,"abandon",{pairs_found:matched,time:elapsed}); };
    window.addEventListener("beforeunload",h);
    return()=>window.removeEventListener("beforeunload",h);
  },[started,matched,elapsed,ch.pairs]);
  ...
  const tapCard = useCallback((id)=>{
    if(locked) return;
    if(!started){ setStarted(true); saveEvent(now,"start",{diff:ch.diff,grid:`${ch.rows}x${ch.cols}`,deck:ch.deck}); }
    setFlipped(prev=>{
              ...
              if(nm===ch.pairs){
                ...
                setTimeout(()=>{
                  setElapsed(e=>{ setMoves(mv=>{
                    const fm=mv+1;
                    saveEvent(now,"complete",{time:e,moves:fm,diff:ch.diff});
  ...
  const handleShare = ()=>{
    ...
    else navigator.clipboard?.writeText(shareText).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2200);});
  };

  const handleLogoTap = ()=>{
    const n=logoTaps+1; setLogoTaps(n);
    if(n>=5){ setShowAdmin(true); setLogoTaps(0); }
  };
  ...
  if(screen==="landing") return (
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:440,margin:"0 auto",padding:"0 16px 48px",minHeight:"100vh"}}>
      ...
      <div style={{textAlign:"center",padding:"44px 0 24px"}}>
        <div style={{fontSize:58,marginBottom:8,cursor:"pointer",WebkitTapHighlightColor:"transparent"}}
          onClick={handleLogoTap} title="tap 5 times for admin">🃏</div>
      ...
      <div style={{border:`2px solid ${DIFF_COLOR[ch.diff]}30`,borderRadius:16,padding:"20px",marginBottom:18,
        background:"var(--color-background-primary)",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          ...
          <div style={{fontSize:12,fontWeight:600,color:DIFF_COLOR[ch.diff],
            background:DIFF_COLOR[ch.diff]+"18",padding:"3px 10px",borderRadius:20}}>{ch.diff}</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7,marginBottom:16}}>
          {[["⊞","Grid",`${ch.rows}×${ch.cols}`],["🃏","Pairs",ch.pairs],
            ["🎨","Theme",ch.deck[0].toUpperCase()+ch.deck.slice(1)]].map(([ic,l,v])=>(
        ...
        <button onClick={()=>setScreen("game")} style={{width:"100%",padding:14,borderRadius:10,border:"none",
          background:DIFF_COLOR[ch.diff],color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer"}}>
      ...
      <div style={{background:"var(--color-background-secondary)",borderRadius:12,padding:"14px 16px"}}>
        <p style={{fontSize:10,fontWeight:500,color:"var(--color-text-secondary)",
          textTransform:"uppercase",letterSpacing:".07em",margin:"0 0 10px"}}>How to play</p>
        {[["👆","Tap any two cards to flip them"],["✅","Matching pair? They stay face-up"],
          ["🧠","No match? Cards flip back — remember where they were!"],
          ["⏱","Match all pairs as fast as you can"],
          ["📊","See how you compare to other players"]].map(([ic,txt])=>(
      ...
      <p style={{textAlign:"center",fontSize:10,color:"var(--color-text-secondary)",opacity:.35,marginTop:12}}>
        Tap 🃏 five times for admin access
      </p>
    </div>
  );
  ...
  if(screen==="result"&&result){
    ...
    return (
      <div style={{fontFamily:"system-ui,sans-serif",maxWidth:440,margin:"0 auto",padding:"20px 16px 44px"}}>
        ...
        <div style={{background:"var(--color-background-secondary)",borderRadius:12,padding:"11px 14px",
          marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:11,color:"var(--color-text-secondary)"}}>Players today</div>
            <div style={{fontSize:20,fontWeight:700,color:"var(--color-text-primary)"}}>{stats.total}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:11,color:"var(--color-text-secondary)"}}>Your speed rank</div>
            <div style={{fontSize:20,fontWeight:700,color:"#3498DB"}}>Top {Math.max(1,100-stats.timePct)}%</div>
          </div>
        </div>

        <div style={{background:"var(--color-background-secondary)",borderRadius:12,padding:"13px",marginBottom:14}}>
          <p style={{fontSize:10,fontWeight:500,color:"var(--color-text-secondary)",
            textTransform:"uppercase",letterSpacing:".07em",margin:"0 0 8px"}}>Share your result</p>
          <pre style={{fontSize:12,color:"var(--color-text-primary)",whiteSpace:"pre-wrap",
            margin:0,fontFamily:"inherit",lineHeight:1.7}}>{shareText}</pre>
        </div>

        <div style={{display:"flex",flexDirection:"column",gap:9}}>
          <button onClick={handleShare} style={{padding:13,borderRadius:10,border:"none",
            background:"#3498DB",color:"#fff",fontSize:15,fontWeight:600,cursor:"pointer"}}>
            {copied?"✅ Copied to clipboard!":"📤 Share Result"}
          </button>
          <button onClick={()=>setScreen("game")} style={{padding:12,borderRadius:10,
            border:"1.5px solid var(--color-border-tertiary)",background:"transparent",
            color:"var(--color-text-primary)",fontSize:14,fontWeight:500,cursor:"pointer"}}>
            🔄 Play Again
          </button>
          <button onClick={()=>setScreen("landing")} style={{padding:11,borderRadius:10,border:"none",
  ...
  return (
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:520,margin:"0 auto",padding:"12px 12px 32px",minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
        <button onClick={()=>{
          if(started&&matched<ch.pairs) saveEvent(now,"abandon",{pairs_found:matched,time:elapsed});
          setScreen("landing");
        }} style={{width:36,height:36,borderRadius:"50%",border:"1px solid var(--color-border-tertiary)",
          background:"var(--color-background-primary)",cursor:"pointer",fontSize:18,
      ...
      <div style={{height:5,background:"var(--color-background-secondary)",borderRadius:3,marginBottom:11,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${(matched/ch.pairs)*100}%`,
          background:DIFF_COLOR[ch.diff],borderRadius:3,transition:"width .4s ease"}}/>
      </div>
      <div style={{textAlign:"center",fontSize:12,color:"var(--color-text-secondary)",marginBottom:12}}>
        {matched} of {ch.pairs} pairs found
      </div>
      ...
      {!started&&(
        <p style={{textAlign:"center",fontSize:12,color:"var(--color-text-secondary)",marginTop:18,opacity:.6}}>
          Tap any card to begin — timer starts on first tap
        </p>
      )}
    </div>
  );
